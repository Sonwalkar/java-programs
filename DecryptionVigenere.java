import java.util.*;
import java.lang.*;
class DecryptionVigenere
{
	public static void main(String args[])
	{
		Scanner sc = new Scanner(System.in);
		String plaintext,key;
		int keylen,plaintextlen,correction;
	...
	//converting to cyper text
		...
		for(int j=0;j<plaintextlen;j++)
		{
			...
			{
				correction=0;
				correction=((ctoipl[j]-keyint[k])-65)%26;
				if(correction<0)
				{
					correction=26+(correction);
					temparr[j]=(correction)+65;
				}
				else
				{
					temparr[j]=correction+65;
			...
			{
				correction=0;
				correction=((ctoipl[j]-keyint[k])-97)%26;
				if(correction<0)
				{
					correction=26+(correction);
					temparr[j]=97+(correction);
				}
				else
				{
					temparr[j]=correction+97;
				}
				k++;
			}
